...
def load_data(strategy: str, preprocessing: str, data_path: str, plot: bool=False):
    #g get conditions data from txt file
    fname_onsets = data_path + 'Onsets/' + os.listdir(data_path + 'Onsets')[0] #get the filename of txt file with onsets
    onsets_all = pd.read_csv(fname_onsets, delimiter='\t', index_col=False, skiprows=4)  # load txt file with onsets
    run_names = [" Pas d'instructions" , " Regulation_3"]
    tr = 2.0000

def get_runs_from_txt(onsets,run_names,tr):
    onsets = onsets_all[['strategy' , 'condition' , 'onsets_seconds']]
    onsets_tr = onsets.copy()  # copy data frame, due to pandas rules

    onsets_tr.loc[ :, ('dur_TR', 'TR')] = np.nan ; onsets_tr.insert(0, 'run', np.nan)

    for row, onset in enumerate(onsets_tr['onsets_seconds']):
        onsets_tr.at[row, 'dur_TR'] = int(round(onset / tr))  # convert seconds to TR todo: not really duration
    n_TR = int(list(onsets_tr['dur_TR'])[-1])  # total number of TR
    colnames = ['run', 'strategy', 'condition', 'TR']  # define variable names
    ...
    i_run = 0
    for row, run in enumerate(onsets_tr['strategy']):
        onsets_tr.at[row, 'run'] = i_run  # set block number
        if row < (n_TR - 1):
            run_next_trial = onsets_tr['strategy'][row + 1]
            if run == run_names[1] and run_next_trial != run_names[1]:
                i_run = i_run + 1
    ...
    colnames = ['block', 'strategy', 'condition', 'TR']  # define variable names
    conds_fmri = pd.DataFrame(index=range(n_TR), columns=colnames)  # set up data frame
    conds_fmri['TR'] = range(n_TR)  # fill in one TR per row
    for row, TR in enumerate(conds_fmri['TR']):
        # find (closest previous) condition for each TR
        diff_to_tr = np.array(
            [TR - dur_TR for dur_TR in onsets_tr['dur_TR']])  # compare TR of each row to TRs in original table
...
W_directory = '/Users/pamelapindi/Documents/Thèse/NF-BD/Pilot Study/'
subName = 'Pilot-06' ; sesNum = 'ses1/'
data_path = os.path.join(W_directory + subName + os.sep + sesNum)


    # define functions
    def load_data(strategy: str, preprocessing: str, data_path: str, plot: bool = False):
        # get conditions data from txt file
        fname_onsets = data_path + 'Onsets/' + os.listdir(data_path + 'Onsets')[
            0]  # get filename of text file with onsets (output from opennft)
        onsets_all = pd.read_csv(fname_onsets, delimiter='\t', index_col=False, skiprows=4)  # load txt file with onsets
        onsets = onsets_all[onsets_all['strategie'] == strategy]
        cond_names = [' neutre',
                      ' regulation']  # set names of two main conditions (like in txt file) ! pay attention to space bar before word (coming from text file)
        tr = 2  # set TR (in seconds)
        conds_fmri = get_conds_from_txt(onsets, cond_names,
                                        tr)  # convert onset data to conditions file (['block', 'condition', 'TR'] with one row per TR)
        conds_fmri = conds_fmri.iloc[list(range(0,
                                                215))]  # todo: only to correct for final onset (and for debugging, to align phantom pilot onsets with pilot 2 data); ! remove later !
        ...
        fmri_data = index_img(fmri_data, range(
            len(conds_fmri)))  # cut brain data to onsets (cut off last scale tr's that are variable and cannot be read from onsets.txt)
    ...
    def load_brain_data(preprocessing, data_path):
        ...
        fname_anat = data_path + 'T1/' + \
                     [item for item in fnames_anat if item.startswith('2') and item.endswith('.nii')][0]
        return fmri_data, fname_anat

    def perform_decoding_cv(conditions, fmri_niimgs, mask, conds_fmri, condition_mask, random_state: int, cv_type: str,
                            n_folds: int, anova: bool):
        ...
        # determine cv method
        if cv_type == 'k_fold':
            cv = RepeatedKFold(n_splits=n_folds, n_repeats=5,
                               random_state=random_state)  # todo: add n_repeats as input option?
            scoring = 'accuracy'
            groups = None
        ...
        decoder = Decoder(estimator='svc', mask=mask, cv=cv, screening_percentile=screening_percentile,
                          scoring=scoring, smoothing_fwhm=smoothing_fwhm,
                          standardize=True)  # todo: discuss settings with Pauline
        # fit decoder
